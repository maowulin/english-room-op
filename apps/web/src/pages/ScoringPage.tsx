...
export function ScoringPage() {
  ...
  const tasks = useMemo(() => {
    ...
    if (!normalizedSearch) return data?.tasks ?? [];
    return data?.tasks.filter((task) => `${task.id} ${task.roomId ?? ''} ${task.playerLabel} ${task.audioAssetId ?? ''}`.toLowerCase().includes(normalizedSearch)) ?? [];
  }, [data?.tasks, search]);
  ...
  const onRetryTask = useCallback(async (task: ScoringTask) => {
    ...
    try {
      ...
      setFeedback(result.status === 'pending' ? '重试请求已记录，评分尚未开始。' : result.message);
    } catch {
  ...
  return <OpsQueryStatus loading={loading} error={error} loadingLabel="加载评分任务…" onRetry={retry}>{data ? <div className="page-stack">
    <header className="page-header"><div><p className="eyebrow">English Room / Scoring pipeline</p><h1 className="page-title">评分任务</h1><p className="page-subtitle">追踪音频、评分任务与报告状态</p></div><div className="page-actions"><label className="search-control"><Icon name="search" size={18} /><input name="scoring-search" aria-label="搜索任务、房间或玩家" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索任务、房间或玩家" /></label><span className="environment-chip"><Icon name="server" size={16} /> Production</span></div></header>
    {data.dataSource === 'demo' ? <p className="data-source-note">数据来源：演示数据（Mock 适配器）</p> : null}
    <section className="metric-grid metric-grid--five"><MetricCard label="处理中" value={formatCount(data.tasks.filter((task) => task.status === 'running').length)} icon="play" values={data.dataSource === 'demo' ? [3, 4, 3, 5, 4, 6, 5] : undefined} /><MetricCard label="等待中" value={formatCount(data.tasks.filter((task) => task.status === 'queued').length)} icon="clock" tone="amber" values={data.dataSource === 'demo' ? [4, 5, 4, 6, 5, 7, 6] : undefined} /><MetricCard label="今日成功" value={formatCount(data.tasks.filter((task) => task.status === 'succeeded').length)} icon="check" values={data.dataSource === 'demo' ? [4, 5, 6, 5, 7, 8, 7] : undefined} /><MetricCard label="今日失败" value={formatCount(data.tasks.filter((task) => task.status === 'failed' || task.status === 'retryable').length)} icon="alert" tone="coral" values={data.dataSource === 'demo' ? [3, 4, 3, 5, 4, 3, 4] : undefined} /><MetricCard label="成功率" value={data.tasks.length ? `${Math.round(data.tasks.filter((task) => task.status === 'succeeded').length / data.tasks.length * 100)}%` : '—'} delta={data.dataSource === 'demo' ? '本地统计' : undefined} icon="chart" values={data.dataSource === 'demo' ? [6, 7, 6, 8, 7, 9, 8] : undefined} /></section>
    ...
    <Panel title="任务队列"><ScoringTable tasks={tasks} httpMode={httpMode} pendingId={pendingId} onRetry={onRetryTask} /></Panel>
