...
export function RoomsPage() {
  ...
  return <OpsQueryStatus loading={loading} error={error} loadingLabel="加载房间列表…" onRetry={retry}>{data ? <div className="page-stack">
    ...
    <section className="metric-grid metric-grid--four"><MetricCard label="进行中" value={formatCount(data.rooms.filter((room) => room.status === 'active').length)} icon="play" values={data.dataSource === 'demo' ? [4, 3, 5, 4, 6, 5, 7] : undefined} /><MetricCard label="等待中" value={formatCount(data.rooms.filter((room) => room.status === 'waiting').length)} icon="clock" tone="amber" values={data.dataSource === 'demo' ? [2, 2, 3, 2, 4, 3, 3] : undefined} /><MetricCard label="今日完成" value={formatCount(data.rooms.filter((room) => room.status === 'ended').length)} icon="check" values={data.dataSource === 'demo' ? [4, 5, 4, 6, 5, 7, 6] : undefined} /><MetricCard label="异常房间" value={formatCount(data.rooms.filter((room) => room.status === 'closing' || room.recordingStatus === 'failed').length)} icon="alert" tone="coral" values={data.dataSource === 'demo' ? [2, 3, 2, 4, 3, 2, 3] : undefined} /></section>
