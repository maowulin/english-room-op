...
export function AuditPage() {
  ...
  const entries = useMemo(() => data?.entries.filter((entry) => (resultFilter === 'all' || entry.result === resultFilter) && (actionFilter === 'all' || entry.action === actionFilter)) ?? [], [actionFilter, data?.entries, resultFilter]);
  ...
  return <OpsQueryStatus loading={loading} error={error} loadingLabel="加载审计日志…" onRetry={retry}>{data ? <div className="page-stack">
    <header className="page-header"><div><p className="eyebrow">English Room / Governance</p><h1 className="page-title">审计日志</h1><p className="page-subtitle">记录所有管理员查询与写操作</p></div><div className="page-actions"><label className="search-control"><Icon name="search" size={18} /><input name="audit-search" aria-label="搜索管理员、操作或资源" placeholder="搜索管理员、操作或资源" /></label><button type="button" className="btn btn-secondary"><Icon name="download" size={16} /> 导出记录</button></div></header>
    {data.dataSource === 'demo' ? <p className="data-source-note">数据来源：演示数据（Mock 适配器）</p> : null}
    <section className="metric-grid metric-grid--four"><MetricCard label="今日操作" value={formatCount(data.entries.length)} delta="+18.6% ↑" icon="chart" values={[5, 6, 5, 7, 6, 8, 7]} /><MetricCard label="写操作" value={formatCount(data.entries.filter((entry) => entry.action.includes('retry') || entry.action.includes('write')).length)} delta="+12.5% ↑" icon="list" values={[3, 4, 3, 5, 4, 6, 5]} /><MetricCard label="失败请求" value={formatCount(data.entries.filter((entry) => entry.result === 'failure').length)} delta="-18.2% ↓" tone="coral" icon="alert" values={[5, 4, 5, 3, 4, 2, 3]} /><MetricCard label="高风险操作" value={formatCount(data.entries.filter((entry) => entry.risk === 'high').length)} delta="需要关注" tone="amber" icon="shield" values={[2, 3, 2, 4, 3, 4, 5]} /></section>
    <Panel title="操作记录" action={<div className="filter-row"><select aria-label="动作筛选" value={actionFilter} onChange={(event) => setActionFilter(event.target.value)}><option value="all">全部操作</option>{actions.map((action) => <option key={action} value={action}>{action}</option>)}</select><select aria-label="结果筛选" value={resultFilter} onChange={(event) => setResultFilter(event.target.value)}><option value="all">全部结果</option><option value="success">成功</option><option value="failure">失败</option></select></div>}><AuditTable entries={entries} onSelect={setSelectedId} /></Panel>
    <div className="content-grid content-grid--audit"><Panel title="操作类型分布"><OperationBars entries={data.entries} /></Panel><Panel title="最近高风险操作"><RiskList entries={data.entries.filter((entry) => entry.risk === 'high')} /></Panel></div>
...
function AuditTable({ entries, onSelect }: { entries: AuditLogEntry[]; onSelect: (id: string) => void }) {
  ...
  return <div className="table-wrap"><table className="data-table"><thead><tr><th>时间</th><th>管理员</th><th>操作</th><th>资源</th><th>结果</th><th>请求 ID</th><th>详情</th></tr></thead><tbody>{entries.map((entry) => <tr key={entry.id}><td>{entry.occurredAt}</td><td>{entry.actor}</td><td>{entry.action}</td><td>{entry.target}</td><td><StatusBadge tone={entry.result === 'failure' ? 'coral' : 'mint'}>{entry.result === 'failure' ? '失败' : '成功'}</StatusBadge></td><td>{entry.requestId ?? '—'}</td><td><button type="button" className="icon-button" aria-label={`查看审计详情 ${entry.id}`} onClick={() => onSelect(entry.id)}><Icon name="search" size={17} /></button></td></tr>)}</tbody></table><div className="table-footer">共 {entries.length} 条 <span>&lt; 1 &gt;</span></div></div>;
}
