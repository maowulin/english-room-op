...
import { formatCount, formatOptionalPercent, formatPercent, formatUpdatedAt } from '../utils/formatters';
  ...
  return <OpsQueryStatus loading={loading} error={error} loadingLabel="加载总览指标…" onRetry={retry}>
    {metrics ? <div className="page-stack">
      ...
      <section className="metric-grid metric-grid--four">
        <MetricCard label="DAU" value={formatCount(metrics.dau)} delta={metrics.dataSource === 'demo' && metrics.dauDeltaPercent !== undefined ? `${metrics.dauDeltaPercent >= 0 ? '+' : ''}${metrics.dauDeltaPercent.toFixed(1)}% ↑` : undefined} icon="user" values={metrics.dataSource === 'demo' ? [8, 10, 9, 12, 11, 14, 13] : undefined} />
        <MetricCard label="会话数" value={formatCount(metrics.sessionCount)} delta={metrics.dataSource === 'demo' ? '+3.2% ↑' : undefined} icon="users" values={metrics.dataSource === 'demo' ? [4, 5, 6, 5, 8, 7, 9] : undefined} />
        <MetricCard label="房间开始" value={formatCount(metrics.roomStartCount)} delta={metrics.roomConversionRate !== undefined ? `${formatPercent(metrics.roomConversionRate)} 转化` : '暂无数据'} icon="home" values={metrics.dataSource === 'demo' ? [3, 4, 4, 5, 7, 6, 8] : undefined} />
        <MetricCard label="评分查看率" value={formatOptionalPercent(metrics.scoreReportViewRate)} delta={metrics.dataSource === 'demo' && metrics.scoringCompletionRate !== undefined ? `${formatPercent(metrics.scoringCompletionRate)} 完成` : undefined} icon="star" values={metrics.dataSource === 'demo' ? [8, 7, 9, 8, 10, 9, 11] : undefined} />
      </section>
      <div className="content-grid content-grid--overview"><Panel title="活跃用户趋势" className="panel--trend">{metrics.activeTrend.length ? <><div className="chart-legend"><span><i className="legend-line" /> DAU</span></div><TrendChart values={metrics.activeTrend.map((point) => ({ label: point.day, count: point.count }))} label="活跃用户趋势" /></> : <EmptyState title="暂无足够数据" description="后端尚未上报趋势事件。" />}</Panel><Panel title="留存表现"><RetentionHeatmap rows={metrics.retentionHeatmap} /></Panel></div>
      <div className="content-grid content-grid--overview"><Panel title="核心转化漏斗" className="panel--funnel"><Funnel items={metrics.funnel} /></Panel><Panel title="今日系统状态"><SystemStatus dataSource={metrics.dataSource} /></Panel></div>
    </div> : null}
  </OpsQueryStatus>;
}
...
function SystemStatus({ dataSource }: { dataSource: OpsDataSource }) {
  const isDemo = dataSource === 'demo';
  const rows = [
    { label: 'API', value: isDemo ? '99.98%' : '暂无数据', icon: 'server' as const, tone: isDemo ? 'mint' as const : 'neutral' as const },
    { label: 'RTC 入房', value: isDemo ? '98.7%' : '暂无数据', icon: 'chart' as const, tone: isDemo ? 'mint' as const : 'neutral' as const },
    { label: '评分', value: isDemo ? 'Not configured' : '未配置', icon: 'star' as const, tone: 'amber' as const },
    { label: '新增长 Issue', value: isDemo ? '—' : '暂无数据', icon: 'alert' as const, tone: 'neutral' as const },
  ] as const;
  return <div className="system-status">{rows.map((row) => <div className="system-status__row" key={row.label}><span className={`metric-icon metric-icon--${row.tone}`}><Icon name={row.icon} size={18} /></span><span>{row.label}</span><strong>{row.value}</strong>{isDemo ? <MiniStatusBars /> : null}</div>)}</div>;
}
